...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void draw(Graphics g) {
        if(running) {
            ...
            g.drawString("Score: "+ applesEatern, (SCREEN_WIDTH - metrics.stringWidth("Score: "+applesEatern)) / 2, g.getFont().getSize());
        }
        else {
            gameOver(g);
        }
    ...
    public void checkCollision() {
        //Check if head collides with obody
        for(int i = bodyParts; i > 0; i--) {
            if(x[0] == x[i] && y[0] == y[i]) {
                running = false;
            }
        ...
        if(y[0] > SCREEN_HIGHT) running = false;
    }
